...
public function salesReturn($salesId)
{
  $salesInfo=DB::table('sales')
   ...
   ->first();
  $salesProduct=DB::table('sales_products')->where('sale_id',$salesId)
  ->join('products','products.id','=','sales_products.pro_id')
  ->select('sales_products.*','products.name','products.code')
  ->get();

foreach($salesProduct as $product){
  $product_name=$product->name; 
  $product_id=$product->pro_id;
  $sell_price=$product->subtotal;
  $qty=$product->qty;
    Cart::add($product_id, $product_name, $qty, $sell_price);
  }
  return view('admin.modules.sales.salesReturn')->with([
  'salesInfo'=>$salesInfo,
  ]);
} 

//remove  item from sales return form
public function salesReturnRemoveItem($rowId)
{
 Cart::remove($rowId);
 return redirect()->back();
}
