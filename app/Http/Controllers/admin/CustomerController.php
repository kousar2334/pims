...
public function customerDetails($id)
{
  $duePaymentHistory=DB::table('sales_due_returns')->where('customer_id',$id)->get();
   ...
   ->where('sales.customer_id',$id)->where('sales.sales_status','completed')->get();
   $totalShopping=DB::table('sales')->where('customer_id',$id)->sum('grand_total');
   $totalDue=DB::table('sales')->where('customer_id',$id)->where('sales.sales_status','completed')->sum('due');
...
//exportExcelCustomerList
public function exportExcelCustomerList()
{
 //return (new CustomerExport)->download('invoices.pdf', \Maatwebsite\Excel\Excel::DOMPDF);
   return Excel::download(new CustomerExport, 'customer-'.date('Y-d-m').'.xlsx');
}
}
